...
public class XMLTokenizer
{
	...
	//XMLTokenCreator
	public abstract class XMLTokenCreator{
		public XMLToken createXMLToken(String token, int token_type){
			XMLToken xmlToken = xmlTokenFactory(token, token_type);
			return xmlToken;
		}
	}
	//XMLTokenConcreteCreator
	public class XMLTokenConcreteCreator extends XMLTokenCreator{
		protected XMLToken xmlTokenFactory(String token, int token_type){
			return new XMLToken(token, token_type);
		}
	}
	//XMLTokenFactoryMethod 
	public XMLToken xmlTokenFactory(String token, int token_type)
	{
		XMLToken xmlToken = new XMLToken(token, token_type);
		return xmlToken;
	}
	...
	{
		//reader	= bufferedReaderFactory(filename);
		//BufferedReaderProduct
		reader		= BufferedReaderFactory.getBufferedReader(filename);
	}
	...
	public XMLToken getNextToken() throws java.io.IOException
	{
		if (line == null)
		{
			...
			if (line == null)
				return xmlTokenFactory(null, XMLToken.NULL);
		...
		{
			XMLToken	token	= xmlTokenFactory(prolog_start_matcher.group(), XMLToken.PROLOG_START);
			inside_tag		= true;
		...
		{
			XMLToken	token	= xmlTokenFactory(prolog_end_matcher.group(), XMLToken.PROLOG_END);
			inside_tag		= false;
		...
		{
			XMLToken	token	= xmlTokenFactory(tag_close_start_matcher.group(), XMLToken.TAG_CLOSE_START);
			inside_tag		= true;
		...
		{
			XMLToken	token	= xmlTokenFactory(tag_start_matcher.group(), XMLToken.TAG_START);
			inside_tag		= true;
		...
		{
			XMLToken	token	= xmlTokenFactory(tag_end_matcher.group(), XMLToken.TAG_END);
			inside_tag		= false;
		...
		{
			XMLToken	token	= xmlTokenFactory(null_tag_end_matcher.group(), XMLToken.NULL_TAG_END);
			inside_tag		= false;
		...
		if (inside_tag)
		{
			...
			{
				XMLToken	token	= xmlTokenFactory(attribute_matcher.group(), XMLToken.ATTRIBUTE);
			...
			{
				XMLToken	token	= xmlTokenFactory(attribute_value_matcher.group(), XMLToken.ATTRIBUTE_VALUE);
			...
			{
				XMLToken	token = xmlTokenFactory(prolog_identifier_matcher.group(), XMLToken.PROLOG_IDENTIFIER);
			...
			{
				XMLToken	token	= xmlTokenFactory(element_matcher.group(), XMLToken.ELEMENT);
		...
		{
			XMLToken	token	= xmlTokenFactory(value_matcher.group(), XMLToken.VALUE);
		...
		return xmlTokenFactory("", XMLToken.NULL);
	}
	...
	public static void main(String args[])
	{
		for (int i = 0; i < args.length; i++)
		{
			...
			//XmlTokenizerProduct
			try {
				tokenizer	= XMLTokenizerFactory.getXMLTokenizer(args[i]);
